...
while true,
	...
	Vx(2:NX,:) = Vx(2:NX,:) - dt_over_rho_x_dx * ( P(2:NX,:) - P(1:NX-1,:) );
	Vy(:,2:NY) = Vy(:,2:NY) - dt_over_rho_x_dx * ( P(:,2:NY) - P(:,1:NY-1) );

	% Update Sound Pressures
	P(1:NX,1:NY) = P(1:NX,1:NY) - kappa_x_dt_over_dx * ( ( Vx(2:NX+1,:) - Vx(1:NX,:) ) + ( Vy(:,2:NY+1) - Vy(:,1:NY) ) );
